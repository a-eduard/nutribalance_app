...
class _ProfileSettingsScreenState extends State<ProfileSettingsScreen> {
  ...
  // НОВОЕ: Контроллер для длины цикла
  final TextEditingController _cycleLengthController = TextEditingController();

  bool _isFetching = true;
  bool _isLoading = false;
  bool _notificationsEnabled = true; // НОВОЕ: Переменная для пушей
    ...
    try {
      ...
      if (userDoc.exists && mounted) {
        ...
        setState(() {
          ...
          String loadedNick = (userData['nickname'] ?? '')
              .toString()
              .toLowerCase();
  ...
  Future<void> _saveProfile() async {
    ...
    if (newNickname.isNotEmpty) {
      ...
      if (!isUnique) {
        if (mounted)
          setState(() {
            _isLoading = false;
            _nicknameError = "Этот никнейм уже занят";
          });
        return;
      }
    }
    ...
    try {
      ...
      final double? height = double.tryParse(
        _heightController.text.replaceAll(',', '.').trim(),
      );
      final double? weight = double.tryParse(
        _weightController.text.replaceAll(',', '.').trim(),
      );

      // НОВОЕ: Сохраняем длину цикла
  ...
  Future<void> _deleteAccount() async {
    ...
    try {
      ...
      if (e.code == 'requires-recent-login') {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('В целях безопасности перезайдите в аккаунт.'),
  ...
  Widget _buildGenderField() {
    return Padding(
      ...
      child: Row(
        children: [
          const Icon(Icons.wc, color: Colors.grey, size: 20),
          const SizedBox(width: 16),
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
                ...
                SizedBox(
                  ...
                  child: DropdownButtonHideUnderline(
                    child: DropdownButton<String>(
                      ...
                      items: <String>['Мужской', 'Женский', 'не указан']
                          .map(
                          ...
                          Navigator.push(
                            ...
                            MaterialPageRoute(
                              builder: (_) => const P2PChatScreen(
                                ...
                                otherUserName: 'Поддержка NutriBalance',
                              ),
                            ),
                          );
                        },
