...
class _SettingsScreenState extends State<SettingsScreen> {
  ...
  Future<void> _pickImage() async {
    try {
      final XFile? picked = await _picker.pickImage(source: ImageSource.gallery, imageQuality: 85, maxWidth: 1080, maxHeight: 1080);
  ...
  Future<void> _saveChanges() async {
    ...
    try {
      ...
      await FirebaseFirestore.instance.collection('users').doc(uid).update({
        ...
        if (_selectedRole == 'athlete') 'age': int.tryParse(_ageController.text) ?? 0,
        if (_selectedRole == 'athlete') 'height': int.tryParse(_heightController.text) ?? 0,
        if (_selectedRole == 'athlete') 'weight': double.tryParse(_weightController.text.replaceAll(',', '.')) ?? 0.0,
  ...
  Future<void> _deleteAccount() async {
    ...
    try {
      ...
      final subcollections = ['workouts', 'history', 'meals', 'nutrition_goal', 'weight_history', 'custom_exercises', 'ai_chats_trainer', 'ai_chats_dietitian', 'assigned_workouts'];
      ...
      if (e.code == 'requires-recent-login') {
        if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Пожалуйста, выйдите из аккаунта и войдите заново для удаления.', style: TextStyle(color: Colors.white)), backgroundColor: Colors.orangeAccent));
      }
    } catch (e) {}
  }
  ...
  Widget _buildAvatarSection() {
    ...
    return Center(
      child: GestureDetector(
        ...
        child: Stack(
          children: [
            CircleAvatar(radius: 50, backgroundColor: Colors.grey[800], backgroundImage: imageProvider, child: imageProvider == null ? const Icon(Icons.person, size: 50, color: Colors.white) : null),
            Positioned(bottom: 0, right: 0, child: Container(padding: const EdgeInsets.all(6), decoration: const BoxDecoration(color: Color(0xFF9CD600), shape: BoxShape.circle), child: const Icon(Icons.camera_alt, size: 16, color: Colors.black))),
          ],
        ),
      ),
    );
  ...
  Widget _buildInputField(TextEditingController controller, String label, {int maxLines = 1, bool isNumber = false}) {
    return Padding(
      ...
      child: TextFormField(
        controller: controller, maxLines: maxLines, keyboardType: isNumber ? const TextInputType.numberWithOptions(decimal: true) : TextInputType.text,
            ...
            child: Column(
              ...
              children: [
                ...
                if (_registeredRole == 'coach') ...[
                  Row(children: [_buildRoleButton('Атлет', "athlete"), const SizedBox(width: 12), _buildRoleButton('Тренер', "coach")]),
                  const SizedBox(height: 32),
                ],
                ...
                if (_selectedRole == 'athlete') ...[
                  Padding(
                    ...
                    child: DropdownButtonFormField<String>(
                      ...
                      style: const TextStyle(color: Colors.white),
                      decoration: InputDecoration(labelText: 'Пол', labelStyle: const TextStyle(color: Colors.grey), filled: true, fillColor: Colors.black.withOpacity(0.5), border: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide.none)),
                      items: ['Мужской', 'Женский'].map((String value) => DropdownMenuItem<String>(value: value, child: Text(value))).toList(),
                      onChanged: (newValue) => setState(() => _selectedGender = newValue!),
                    ),
                  ),
                  _buildInputField(_ageController, 'Возраст', isNumber: true),
                  _buildInputField(_heightController, 'Рост (см)', isNumber: true),
                  _buildInputField(_weightController, 'Вес (кг)', isNumber: true),
                ],

                const SizedBox(height: 16),
                const Divider(color: Colors.white10),
                ListTile(contentPadding: EdgeInsets.zero, leading: const Icon(Icons.description, color: Colors.grey), title: const Text('Пользовательское соглашение', style: TextStyle(color: Colors.grey, fontSize: 14)), trailing: const Icon(Icons.open_in_new, color: Colors.grey, size: 16), onTap: () => launchUrl(Uri.parse("https://docs.google.com/document/d/1aZNeAoui_eiEuxMTW3KEJNo9fMktMt-esbsrEmKgi6I/edit?usp=sharing"), mode: LaunchMode.externalApplication)),
                ListTile(contentPadding: EdgeInsets.zero, leading: const Icon(Icons.privacy_tip, color: Colors.grey), title: const Text('Политика конфиденциальности', style: TextStyle(color: Colors.grey, fontSize: 14)), trailing: const Icon(Icons.open_in_new, color: Colors.grey, size: 16), onTap: () => launchUrl(Uri.parse("https://docs.google.com/document/d/1LZXjxv2vJYXOkicb_zsul8NM4VNoBAQhuw7hycOiyBQ/edit?usp=sharing"), mode: LaunchMode.externalApplication)),
                const Divider(color: Colors.white10),
                ...
                SizedBox(
                  ...
                  child: ElevatedButton(
                    ...
                    child: _isLoading ? const SizedBox(height: 24, width: 24, child: CircularProgressIndicator(color: Colors.black, strokeWidth: 2)) : const Text('СОХРАНИТЬ', style: TextStyle(color: Colors.black, fontSize: 16, fontWeight: FontWeight.w900)),
                  ),
                ),
                const SizedBox(height: 32),
                ...
                ListTile(
                  ...
                  title: Text('logout_account'.tr(), style: const TextStyle(color: Colors.white54, fontWeight: FontWeight.bold)), 
                  onTap: _logout, // Теперь кнопка ссылается на наш новый безопасный метод
                ),
                ListTile(contentPadding: EdgeInsets.zero, leading: const Icon(Icons.delete_forever, color: Colors.redAccent), title: const Text('Удалить аккаунт', style: TextStyle(color: Colors.redAccent, fontWeight: FontWeight.bold)), onTap: () => _showDeleteAccountDialog(context)),
