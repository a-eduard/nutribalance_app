...
class _HomeTabState extends State<HomeTab> with AutomaticKeepAliveClientMixin {
  ...
  String _getCheatStatus(DateTime day, Map<String, dynamic> userData) {
    if (userData['isPregnant'] == true ||
        userData['goal'] == 'Здоровая беременность')
      return 'none';
    ...
    if (lastStartTs != null) {
      final start = DateTime(
        lastStartTs.toDate().year,
        lastStartTs.toDate().month,
        lastStartTs.toDate().day,
      );
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              const Text(
                "В этот день мы не строги к себе. Ты можешь позволить себе любимые блюда без угрызений совести и жесткого контроля.",
                textAlign: TextAlign.center,
                style: TextStyle(
                  color: _subTextColor,
                  fontSize: 15,
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              const Text(
                "Сегодня твой организм отдыхает. Никакого жесткого подсчета калорий, только гармония и комфорт 🌸",
                textAlign: TextAlign.center,
                style: TextStyle(
                  color: _subTextColor,
                  fontSize: 15,
        ...
        builder: (context, setStateDialog) {
          ...
          return AlertDialog(
            ...
            content: Column(
              ...
              children: [
                const Text(
                  "Измените калории, и БЖУ пересчитаются сами. Или настройте макросы вручную.",
                  style: TextStyle(
                    color: _subTextColor,
                    fontSize: 12,
                    height: 1.4,
              ...
              Padding(
                ...
                child: Row(
                  ...
                  children: [
                    ...
                    // === РЕАКТИВНАЯ КОРЗИНА: УВЕЛИЧЕННАЯ, С КРАСНОЙ ТОЧКОЙ (Requirement 1, 2, 3) ===
                    StreamBuilder<QuerySnapshot>(
                      // Мы слушаем коллекцию 'shopping_list' для текущего пользователя (uid уже определен выше)
                      stream: FirebaseFirestore.instance
                          .collection('users')
                          .doc(uid)
                      ...
                      builder: (context, cartSnapshot) {
                        // ИСПРАВЛЕНО: Проверяем реальное наличие категорий/ингредиентов внутри документов
                        bool hasItems = false;
                        if (cartSnapshot.hasData &&
                            cartSnapshot.data!.docs.isNotEmpty) {
                        ...
                        return Badge(
                          // requirement 3: Исчезает, когда пусто
                          isLabelVisible: hasItems,
                          backgroundColor: Colors.redAccent,
                          smallSize: 10,
                          // requirement 2: Позиционирование (ближе к иконке)
                          // value = y, right = x (x moves left/right, y moves up/down)
                          label: null, // Показываем просто точку
                          offset: const Offset(
                            -2,
                            2,
                          ), // Тянем точку внутрь иконки (влево и вниз)
                          child: IconButton(
                            ...
                            padding: EdgeInsets
                                .zero, // Убираем дефолтные отступы, чтобы Badge ложился лучше
                            constraints:
                                const BoxConstraints(), // Убираем ограничения размера кнопки
                            icon: Image.asset(
                              'assets/icons/bag.png',
                              // requirement 1: Увеличили с 24 до 32
                              width: 32,
                              height: 32,
                              color: _textColor,
                            ),
        ...
        children: List.generate(7, (index) {
          ...
          return GestureDetector(
            onTap: () {
              ...
              if (cheatStatus == 'cheat_meal')
                _showCheatMealSheet();
              else if (cheatStatus == 'cheat_day')
                _showCheatDaySheet();
            },
            child: Column(
              children: [
  ...
  Widget _buildNutritionDashboard(String uid) {
    return StreamBuilder<DocumentSnapshot>(
      ...
      builder: (context, goalSnapshot) {
        ...
        return StreamBuilder<DocumentSnapshot>(
          key: ValueKey('dash_$_docId'), // <-- ФИКС КАЛЕНДАРЯ
          stream: FirebaseFirestore.instance
              .collection('users')
              .doc(uid)
              .collection('meals')
          ...
          builder: (context, mealSnapshot) {
            ...
            if (mealSnapshot.hasData && mealSnapshot.data!.exists) {
              ...
              final List<dynamic> activeItems = rawItems
                  .where(
                    (item) =>
                        !_optimisticDeletedIds.contains(item['id'].toString()),
                  )
                  .toList();
              ...
              for (var item in activeItems) {
                if (item is Map<String, dynamic>) {
                  ...
                  if (item.containsKey('health_score') &&
                      item['health_score'] != null) {
                    totalHealthScore += (item['health_score'] as num)
                        .toDouble();
                    mealsCount++;
                  }
                }
            ...
            final int avgHealthScore = mealsCount > 0
                ? (totalHealthScore / mealsCount).round()
                : 0;
            targetCals += bonusCals;
            bool isExceeded = targetCals > 0 && curC > targetCals;
            final Color activeColor = isExceeded
                ? _lavenderColor
                : _accentColor;
              ...
              child: Container(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 4),
                    Text(
                      isExceeded ? 'Сверх нормы ✨' : 'Калорий употреблено',
                      style: TextStyle(
                        color: isExceeded
                            ? const Color(0xFFB6A6CA)
                            : _subTextColor,
        ...
        children: [
          ...
          TweenAnimationBuilder<Color?>(
            ...
            builder: (context, color, child) {
              // ОБНОВЛЕНИЕ ДИЗАЙНА:
              // - minHeight увеличен до 8 для большей "массивности"
              // - BorderRadius.circular(10) для создания мягкой капсульной формы
              return ClipRRect(
                ...
                child: LinearProgressIndicator(
                  ...
                  backgroundColor: const Color(
                    0xFFF2F2F7,
                  ), // Сделал фон чуть темнее, чтобы контраст был лучше
                  valueColor: AlwaysStoppedAnimation<Color>(
                    color ?? _accentColor,
                  ),
                  minHeight: 8, // <-- Сделали полоску толще
                ),
              );
            },
          ),
  ...
  Widget _buildDiaryList(String uid) {
    return StreamBuilder<DocumentSnapshot>(
      key: ValueKey('diary_$_docId'), // <-- ФИКС КАЛЕНДАРЯ
      stream: FirebaseFirestore.instance
          .collection('users')
          .doc(uid)
          .collection('meals')
      ...
      builder: (context, snapshot) {
        if (snapshot.hasError)
          return const Center(child: Text("Ошибка загрузки дневника"));
        ...
        final List<dynamic> items = rawItems
            .where(
              (item) => !_optimisticDeletedIds.contains(item['id'].toString()),
            )
            .toList();
          ...
          itemBuilder: (context, index) {
            ...
            return Dismissible(
                ...
                return await showDialog<bool>(
                      ...
                      builder: (BuildContext context) {
                        return AlertDialog(
                          ...
                          content: const Text(
                            "Ты уверена, что хочешь удалить этот прием пищи?",
                            style: TextStyle(
                              color: _subTextColor,
                              fontSize: 15,
                            ),
              ...
              onDismissed: (_) {
                ...
                DatabaseService()
                    .deleteMealItem(item, List.from(rawItems), _docId)
                    .catchError((e) {
                      debugPrint(
                        "Фоновая БД тупит: $e",
                      ); // Молча игнорируем, не пугаем юзера.
                    });
                ...
                onTap: () {
                  ...
                  if (isGrouped)
                    Navigator.push(
                      ...
                      MaterialPageRoute(
                        builder: (_) =>
                            MealDetailScreen(mealData: item, dateDocId: _docId),
                      ),
                    );
                  else
                  ...
                  child: Row(
                    ...
                    children: [
                      Container(
                        width: 56,
                        height: 56,
                        // Делаем нежный розовый фон для заглушки
                        decoration: BoxDecoration(
                          color: isValidUrl ? _bgColor : const Color(0xFFFDECE8),
                          borderRadius: BorderRadius.circular(16),
                        ),
                        child: ClipRRect(
                          borderRadius: BorderRadius.circular(16),
                          child: isValidUrl
                              ...
                              : (imageUrl != null && imageUrl.startsWith('assets/'))
                                  ? Padding(
                                      // Добавляем отступы, чтобы картинка не обрезалась
  ...
  Widget _buildWaterWidget(String uid) {
    return OptimisticWaterWidget(
      key: ValueKey('water_$_docId'),
      uid: uid,
      docId: _docId,
    );
  }
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 8),
          Text(
            "Нажми на кнопку внизу экрана,\nчтобы записать свой первый прием",
            textAlign: TextAlign.center,
  ...
  Widget build(BuildContext context) {
    return StreamBuilder<DocumentSnapshot>(
      ...
      builder: (context, snapshot) {
        ...
        if (snapshot.hasData && snapshot.data!.exists) {
          int serverGlasses =
              (snapshot.data!.data() as Map<String, dynamic>?)?['water_glasses']
                  ?.toInt() ??
              0;
          if (!_isOptimistic) {
